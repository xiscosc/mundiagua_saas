...
from .views import SearchClientView, CreateAthRepairView, CreateIdegisRepairView, AthRepairView, IdegisRepairView, \
    ...
    LinkInterventionView, UnlinkInterventionView, CreateZodiacRepairView, ZodiacRepairView
...
urlpatterns = [
    ...
    url(r'^list/(?P<type>all|idegis|ath|zodiac)/(?P<status_id>\d+)/(?P<budget>\d+)/(?P<starred>\d+)/$', ListRepairView.as_view(), name="repair-list"),
    url(r'^psearch/$', PreSearchRepairView.as_view(), name="repair-psearch"),
    url(r'^search/(?P<type>all|idegis|ath|zodiac)/(?P<starred>\d+)/$', SearchRepairView.as_view(), name="repair-search"),
    url(r'^print/(?P<logo>\d+)/(?P<type>idegis|ath|zodiac)/(?P<pk>\d+)/$', PrintRepairView.as_view(), name="repair-print"),
    url(r'^starred/(?P<type>idegis|ath|zodiac)/(?P<pk>\d+)/$', ToggleStarredRepairView.as_view(), name="repair-starred"),
    url(r'^link/(?P<pk>\d+)/(?P<type>idegis|ath|zodiac)/$', LinkInterventionView.as_view(), name="repair-link-intervention"),
    url(r'^unlink/(?P<pk>\d+)/(?P<type>idegis|ath|zodiac)/(?P<pk_intervention>\d+)/(?P<to_repair>\d+)/$', UnlinkInterventionView.as_view(),
